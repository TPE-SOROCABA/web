...
export const WeekDesignation = () => {
  ...
  const { designationId, participantId } = useParams();
  const http = useHttp();
  const [page, setPage] = useState<"loading" | "screen" | "not-found">(
    "loading"
  );
  ...
  const getDesignation = useCallback(async () => {
    const url = participantId
      ...
      : `/designations/${designationId}/participants`;
    try {
  ...
  }, [designationId, http, participantId]);
  ...
  const handleAbsent = async (reason: string) => {
    await toast.promise(
      http.post(`/participants/${participantId}/incidences`, { reason }),
      {
        ...
        <div className="flex flex-col justify-center items-center relative gap-1 max-w-[500px] m-auto">
          ...
          <p className="text-center p-2 text-sm">
            ...
            {participantId ? (
              ...
              <>
                <strong>Designações da Semana</strong>
                <br></br>
                ...
                return (
                  <div key={index} className="flex flex-col gap-1">
                    ...
                    <div className="flex justify-between gap-2 text-md">
                      <strong className="truncate w-[80%]">
                        {designation.point}
                      </strong>
