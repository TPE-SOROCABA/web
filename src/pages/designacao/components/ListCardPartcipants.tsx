...
export function ListaCardsParticipantes({
  ...
  return (
    <div className="grid grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 2xl:grid-cols-7 gap-8 justify-around w-full">
      {participants
        .filter((p) => p.name)
        .map((participant) => (
          <ParticipantCard
...
function ParticipantCard({
  ...
  return (
    <div
      className="w-56 h-40 col-span-1 border rounded-lg shadow-md bg-gray-50 relative flex flex-col justify-between"
      key={participant.id}
    >
      ...
      <div className="px-4 text-sm text-gray-800">
        <span>
          {participant.incident_history && participant.incident_history.reason}
        </span>
      </div>
          ...
          <div className="flex justify-between items-center flex-col gap-2 bg-white p-4 rounded-lg min-w-[20vw] max-w-96">
            ...
            <div className="flex justify-between items-center mt-4 w-full gap-2">
              ...
              <Button
                ...
                onClick={() => {
                  if (!reason)
                    return toast.error("Informe o motivo da ausência");
                  if (reason.length < 3)
                    return toast.error("Motivo muito curto");
