...
export const useDesignation = () => {
    ...
    const handleUpdatePoint = async (pointId: string, status: boolean) => {
      await toast.promise(http.patch<Designation>(`/designations/${desigantion!.id}/points/${pointId}`, {
        status
      }), {
    ...
    const handleUpdatePointParticipants = async (pointId: string, participantIds: string[]) => {
      await toast.promise(http.put<Designation>(`/designations/${desigantion!.id}/points/${pointId}/participants`, {
    ...
    const createIncidentParticipants = async (participantId: string) => {
      await toast.promise(http.post<Designation>(`/participants/${participantId}/incidences`, {
